...
| Client uploads message or revision | Pass for message path | Message submit has a busy state. Related request and deliverable references are validated. Shortcut buttons open the Messages and Files panel, the form layout stays contained, and the admin dossier shows the submitted message body. Revision file upload still needs a live browser file picker pass. |
